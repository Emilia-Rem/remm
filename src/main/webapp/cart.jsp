...
<script type="text/javascript">
	function pNum(pid,p,no){
	    var num = $("#num_count"+no).val();
	    var number = Number(num);
	    number++;
		$.ajax({
			url:"${pageContext.request.contextPath}/cart?method=addCartAjax&goodsId="+pid+"&number="+(number)+"&price="+(p*number),
			method:"get",
			success:function(){
				location.href = "${pageContext.request.contextPath}/cart?method=getCart";
			},
			error:function(){
				alert("服务器异常");
			}
	...
	function mNum(pid,p,no){
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/cart?method=addCartAjax&goodsId="+pid+"&price="+(p*number)+"&number="+(number),
			method:"get",
			success:function(){
				location.href = "${pageContext.request.contextPath}/cart?method=getCart";
			},
	...
	function deleteCart(pid){
		if(confirm("确认要删除吗")){
			$.ajax({
				url:"${pageContext.request.contextPath}/cart?method=addCartAjax&goodsId="+pid+"&number=0",
				method:"get",
				success:function(){
					location.href = "${pageContext.request.contextPath}/cart?method=getCart";
				},
	...
	function clearCart(){
		if(confirm("确定要清空吗")){
			$.ajax({
				url:"${pageContext.request.contextPath}/cart?method=clearCartAjax",
				method:"get",
				success:function(){
					location.href = "${pageContext.request.contextPath}/cart?method=getCart";
				},
	...
	<div class="row">
		<div class="pull-right" style="margin-right: 40px;">
	            ...
	            <div>
	            	...
	            	&nbsp;&nbsp;
	            	<a href="${pageContext.request.contextPath}/order?method=getOrderView" class="btn  btn-danger btn-lg">结账</a>
